...
## 12) Solución de error común en Vite + Vue

Si al ejecutar `npm run dev` aparece este error:
...
Significa que falta configurar el plugin de Vue en Vite.

Verifica estos dos puntos en `frontend/`:

1. `package.json` debe tener `@vitejs/plugin-vue` en `devDependencies`.
2. Debe existir `vite.config.js` con:
...
Luego vuelve a instalar dependencias y levantar frontend:
